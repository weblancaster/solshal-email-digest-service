...
function sendCustomizedEmail(user) {
  ...
  //     subject: `Hey! ${user.name} your latest bookmarks are here.`,
    ...
    .then((users) => {
      ...
      formattedUsers = _.map(users, (user) => {
        ...
        flattenedUserCollection = _.filter(_.flattenDeep(userCollections), (collection) => {
          // collection created date is older than 6 days ago?
          return moment(collection.created_at).format() > moment(Date.now()).subtract(6, 'days').format();
        });
